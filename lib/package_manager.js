...
/**
 * Concatenates all 'bundleable' files and copies the remaining files that can't be bundled.
...
const bundleResources = (sourcePath, output, bundleManifest) =>
  new Promise((resolve, reject) => {
    ...
    const filesToCopy = (bundleManifest.unbundledResources || []).map(
      resource => ({
        source: path.join(sourcePath, buildRelativeResourcePath(resource)),
    ...
    .then(() => utils.concatFiles(path.join(output.path, output.fileName), filesToBundle))
