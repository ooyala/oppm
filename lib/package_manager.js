...
const bundleResources = (sourcePath, output, bundleManifest) =>
  new Promise((resolve, reject) => {
    // The resources that have `bundlePath` set need to be copied to a different
    // path (usually the root) in order to work properly with the concetenated bundle file.
    // For these resources we set the copy paths explicilty
    const copyPatterns = bundleManifest.unbundledResources.filter(
      resource => typeof resource.bundlePath !== 'undefined'
    ).map(
      resource => ({
        context: sourcePath,
        from: buildRelativeResourcePath(resource),
        to: buildRelativeResourcePath(resource, true)
      })
    );
    // This will copy all the remaining unbundled files so long as they aren't
    // matched by the ignore pattern that is populated below
    copyPatterns.push({ from: sourcePath });
    ...
    const options = {
      ...
      copy: {
        ...
        options: {
          ignore: bundleManifest.bundledResources.map(
            resource => buildRelativeResourcePath(resource)
          )
        }
      }
    ...
    const compiler = webpack(webpackConfig(sourcePath, output, options));
    ...
    compiler.run((err, stats) => {
      ...
      const packagedResources = [{
        path: '',
        fileName: `${stats.hash}.${output.fileName}`
