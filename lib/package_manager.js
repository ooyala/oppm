...
const generateManifest = (destPath, fileName, resources) =>
  new Promise((resolve, reject) => {
    ...
    file.on('close', () => resolve(bundledFiles));
    file.on('error', err => reject(new Error(`Failed to create package manifest: ${err.message}`)));

    let relativeResourcePath = '';
    resources.forEach((resource) => {
      if (resource.fileName.match(/\.(min\.js|css)$/)) {
        relativeResourcePath = buildRelativeResourcePath(resource);
        bundledFiles.push(relativeResourcePath);
        file.write(`require('./${relativeResourcePath}');\n`);
      }
    });
    file.end();
  });
 ...
 * @param {object} bundlingOptions
 * @returns {Promise} A promise that will be resolved with an array that contains all the paths of the files that were chosen to be bundled together
 */
const bundleResources = (entry, output, bundlingOptions) => {
  let options = {
    excludeFromCopy: []
  };
  options = Object.assign({}, options, bundlingOptions);
