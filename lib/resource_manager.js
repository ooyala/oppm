...
const filterPluginGroupResources = (pluginGroup, version, buildType, filterIds) => {
  ...
  const addResource = (resources, pluginResource, fileName) => {
    ...
    resources.push({
      ...
      bundlePath: pluginResource.bundlePath,
      ...
      excludeFromPage: !!pluginResource.excludeFromPage
    });
  };
 ...
 * @param {boolean} useBundlePath Use the special `bundlePath` property when building instead of the regular `path`
 * @returns {string} The relative path to the resource
 */
const buildRelativeResourcePath = (resource, useBundlePath) => {
  if (!resource) {
    return '';
  }
  let resourcePath = '';
  // Use bundle path if requested, but only when available
  if (useBundlePath) {
    resourcePath = (typeof resource.bundlePath === 'undefined') ? resource.path : resource.bundlePath;
  } else {
    resourcePath = resource.path;
  }
  return `${resourcePath ? `${resourcePath}/` : ''}${resource.fileName}`;
};
